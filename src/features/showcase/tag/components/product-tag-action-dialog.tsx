...
import { closeShowcaseDialog, resetCategoryDialogs, resetProductTagDialogs, setSelectedProductCategoryRow } from '@/stores/actions/showcase-actions'
import { Textarea } from '@/components/ui/textarea'
import { useEffect, useState } from 'react'
import { createProductTag, updateProductTag } from '@/api/showcase/product-tag.service'
...
export function ProductTagActionDialog({
  currentRow,
  open,
  onOpenChange,
  ...
  const selectedRow = useShowcaseStore(state=>state.selectedProductTagRow);
  ...
  const onSubmit = (values: UserForm) => {
   ...
   if(isEdit){
      ...
      updateProductTagMutation.mutate({
        data:{ 
          ...base,
          ...(imageFile && { image: imageFile }),
        },
        tagId: selectedRow?._id as string
      })

      return;
   }

    createProductTagMutation.mutate({
      data:{
        ...base,
    ...
    >
      <DialogContent className='sm:max-w-lg'>
        <DialogHeader className='text-start'>
          <DialogTitle>{isEdit ? 'Edit Product Tag' : 'Add New Product Tag'}</DialogTitle>
          <DialogDescription>
            {isEdit ? 'Update the Product Tag here. ' : 'Create new Product Tag here. '}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className='grid grid-cols-6 items-center space-y-0 gap-x-4 gap-y-1'>
                    ...
                    <FormControl>
                      <Textarea
                        placeholder='Type the description of the Product Tag'
