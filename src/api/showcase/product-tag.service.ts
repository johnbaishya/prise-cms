...
import { createProductTagDTO, ListProductTagQueryDTO, updateProductTagDTO } from "@/Types/request/showcase-request";
import { apiClient } from "../apiClient";
import ENDPOINTS from "../endpoints";
...
export function getProductTagList(query:ListProductTagQueryDTO,companyId:string) {
  ...
  return apiClient.get(ENDPOINTS.showcase.listProductTagByCompanyId(companyId, queryParams)).then(res => res.data)
}
...
export function updateProductTag(param:{data:updateProductTagDTO,tagId:string}){
  let fd = new FormData();
