...
export function getProductCategoryList(query:ListProductCategoryQueryDTO,companyId:string) {
  ...
  return apiClient.get(ENDPOINTS.showcase.listProductCategoryByCompanyId(companyId, queryParams)).then(res => res.data)
}
...
export function updateProductCategory(param:{data:updateProductCategoryDTO,categoryId:string}){
  let fd = new FormData();
  ...
  return apiClient.put(ENDPOINTS.showcase.updateProductCategory(param.categoryId),fd);
}
...
export function createProductCategory(param:{data:createProductCategoryDTO}){
  const data = param.data;
